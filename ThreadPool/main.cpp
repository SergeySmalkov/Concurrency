...
int main() {
    ...
    size_t id_previous5 = tp.Submit([&mt, first_id](){
        size_t j = 0;
        ...
        SetResult(j);
        std::lock_guard<std::mutex> lk(mt);
        std::cout << j << " kekeke5" << std::endl;
    });

    size_t id_previous4 = tp.Submit([&mt, id_previous5](){
        size_t j = 0;
        for(size_t i = 0; i < 100; ++i){
            ++j;
        }
        ...
        //Wait(id_previous5);
        j = std::any_cast<size_t>(GetResult(id_previous5));
        std::this_thread::sleep_for(std::chrono::seconds(3));
        std::lock_guard<std::mutex> lk(mt);
        std::cout << j << " kekeke4" << std::endl;
    });

    size_t id_previous3 = tp.Submit([&mt, id_previous4](){
        size_t j = 0;
        for(size_t i = 0; i < 100; ++i){
            ++j;
        }
